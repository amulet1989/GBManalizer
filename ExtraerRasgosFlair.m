function [Rasgos]=ExtraerRasgosFlair(Ibox,ROIonly,levels)
Rasgos=zeros(1,7);
Ibox1=Ibox';
IVect=[Ibox1(:)]';
%% 1st order descriptive statistics
% Rasgos(1)=range(IVect);%ok(1)
...
%Globals
Nbins=length(levels);
CaractGlobal=getGlobalTexturesFlair(ROIonly,Nbins);%Ibox
%4
Rasgos(3)=CaractGlobal.Variance;
%5
Rasgos(4)=CaractGlobal.Skewness;
%6
% Rasgos(6)=CaractGlobal.Kurtosis;

%% Local texture
GLCMs=getGLCM(ROIonly,levels);
CaractH1=getGLCMtexturesFlair(GLCMs(:,:,1));
%7
...
%% Global
function [textures] = getGlobalTexturesFlair(ROIonly,Nbins)
% -------------------------------------------------------------------------
...
%% Textura
function [textures] = getGLCMtexturesFlair(GLCM)
% -------------------------------------------------------------------------
