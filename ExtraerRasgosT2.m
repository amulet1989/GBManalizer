function [Rasgos]=ExtraerRasgosT2(Ibox,ROIonly,levels)
Rasgos=zeros(1,7);
Ibox1=Ibox';
IVect=[Ibox1(:)]';
%% 1st order descriptive statistics
% Rasgos(1)=range(IVect);%ok(4)
Rasgos(1)=max(IVect);%ok(2)
Rasgos(2)=min(IVect);%ok(3)

%% Texture statistics 2nd order GLCM

%Globals
Nbins=length(levels);
CaractGlobal=getGlobalTexturesT2(ROIonly,Nbins);%Ibox
%4
Rasgos(3)=CaractGlobal.Variance;%4
%5
Rasgos(4)=CaractGlobal.Skewness;%5
%6
% Rasgos(6)=CaractGlobal.Kurtosis;%6

%% Textura locales
GLCMs=getGLCM(ROIonly,levels);
CaractH1=getGLCMtexturesT2(GLCMs(:,:,1));
%7
% Rasgos(7)=CaractH1.Energy;
%8
% Rasgos(8)=CaractH1.Contrast;
%9
% Rasgos(4)=CaractH1.Entropy;
%10
% Rasgos(10)=CaractH1.Homogeneity;
%11
Rasgos(5)=CaractH1.Correlation;
Rasgos(6)=CaractH1.SumAverage; %12
% Rasgos(8)=CaractH1.Variance;   %13
% Rasgos(9)=CaractH1.Dissimilarity;%14
Rasgos(7)=CaractH1.AutoCorrelation;%15
end
%% Global
function [textures] = getGlobalTexturesT2(ROIonly,Nbins)
% -------------------------------------------------------------------------
...
%% Textura
function [textures] = getGLCMtexturesT2(GLCM)
% -------------------------------------------------------------------------
...
% 12. SumAverage, Ref.[2]; and 13. Variance, Ref.[2]. (adapted from Variance and SumAverage metrics defined by Haralick in Ref. [1])
